...
int main(int argc, char *argv[])
{
    ...
    while (t < steps)
    {
        ...
        if (up)
        {
            ...
            if (intra_rank == 0)
                MPI_Isend(gather_buf_up, pos, MPI_DOUBLE, myrank - Px, myrank, MPI_COMM_WORLD, &request);
        }
        ...
        if (bot)
        {
            ...
            if (intra_rank == 0)
                MPI_Isend(gather_buf_bot, pos, MPI_DOUBLE, myrank + Px, myrank, MPI_COMM_WORLD, &request);
        }
        ...
        if (left)
        {
            pos = 0;
            for (i = 0; i < n; i++)
                MPI_Pack(&buf_1[i][0], 1, MPI_DOUBLE, send_left_2, 2 * n * sizeof(double), &pos, MPI_COMM_WORLD);
            for (i = 0; i < n; i++)
                MPI_Pack(&buf_1[i][1], 1, MPI_DOUBLE, send_left_2, 2 * n * sizeof(double), &pos, MPI_COMM_WORLD);
        ...
        if (right)
        {
            pos = 0;
            for (i = 0; i < n; i++)
                MPI_Pack(&buf_1[i][n - 1], 1, MPI_DOUBLE, send_right_2, 2 * n * sizeof(double), &pos, MPI_COMM_WORLD);
            for (i = 0; i < n; i++)
                MPI_Pack(&buf_1[i][n - 2], 1, MPI_DOUBLE, send_right_2, 2 * n * sizeof(double), &pos, MPI_COMM_WORLD);
        ...
        {
            if (intra_rank == 0)
                MPI_Recv(recv_gather_up, 2 * n * Px * sizeof(double), MPI_DOUBLE, myrank - Px, myrank - Px, MPI_COMM_WORLD, &status);
        ...
        {
            if (intra_rank == 0)
                MPI_Recv(recv_gather_bot, 2 * n * Px * sizeof(double), MPI_DOUBLE, myrank + Px, myrank + Px, MPI_COMM_WORLD, &status);
        ...
        if (left)
        {
            ...
            for (i = 0; i < 2 * n; i++)
                MPI_Unpack(recv_left_2, 2 * n * sizeof(double), &pos, arr_left_2 + i, 1, MPI_DOUBLE, MPI_COMM_WORLD);
        }
        if (right)
        {
            pos = 0;
            ...
            for (i = 0; i < 2 * n; i++)
                MPI_Unpack(recv_right_2, 2 * n * sizeof(double), &pos, arr_right_2 + i, 1, MPI_DOUBLE, MPI_COMM_WORLD);
        }
    ...
    while (t < steps)
    {
        ...
        if (up)
        {
            pos = 0;
            for (i = 0; i < n; i++)
                MPI_Pack(&buf_0[0][i], 1, MPI_DOUBLE, send_up_2, 2 * n * sizeof(double), &pos, MPI_COMM_WORLD);
            for (i = 0; i < n; i++)
                MPI_Pack(&buf_0[1][i], 1, MPI_DOUBLE, send_up_2, 2 * n * sizeof(double), &pos, MPI_COMM_WORLD);
        ...
        if (bot)
        {
            pos = 0;
            for (i = 0; i < n; i++)
                MPI_Pack(&buf_0[n - 1][i], 1, MPI_DOUBLE, send_bot_2, 2 * n * sizeof(double), &pos, MPI_COMM_WORLD);
            for (i = 0; i < n; i++)
                MPI_Pack(&buf_0[n - 2][i], 1, MPI_DOUBLE, send_bot_2, 2 * n * sizeof(double), &pos, MPI_COMM_WORLD);
        ...
        if (left)
        {
            pos = 0;
            for (i = 0; i < n; i++)
                MPI_Pack(&buf_0[i][0], 1, MPI_DOUBLE, send_left_2, 2 * n * sizeof(double), &pos, MPI_COMM_WORLD);
            for (i = 0; i < n; i++)
                MPI_Pack(&buf_0[i][1], 1, MPI_DOUBLE, send_left_2, 2 * n * sizeof(double), &pos, MPI_COMM_WORLD);
            MPI_Isend(send_left_2, pos, MPI_PACKED, myrank - 1, myrank, MPI_COMM_WORLD, &request);
        }
        if (right)
        {
            pos = 0;
            for (i = 0; i < n; i++)
                MPI_Pack(&buf_0[i][n - 1], 1, MPI_DOUBLE, send_right_2, 2 * n * sizeof(double), &pos, MPI_COMM_WORLD);
            for (i = 0; i < n; i++)
                MPI_Pack(&buf_0[i][n - 2], 1, MPI_DOUBLE, send_right_2, 2 * n * sizeof(double), &pos, MPI_COMM_WORLD);
            MPI_Isend(send_right_2, pos, MPI_PACKED, myrank + 1, myrank, MPI_COMM_WORLD, &request);
        }
        ...
        if (up)
        {
            ...
            for (i = 0; i < 2 * n; i++)
                MPI_Unpack(recv_up_2, 2 * n * sizeof(double), &pos, arr_up_2 + i, 1, MPI_DOUBLE, MPI_COMM_WORLD);
        }
        if (bot)
        {
            pos = 0;
            ...
            for (i = 0; i < 2 * n; i++)
                MPI_Unpack(recv_bot_2, 2 * n * sizeof(double), &pos, arr_bot_2 + i, 1, MPI_DOUBLE, MPI_COMM_WORLD);
        }
        if (left)
        {
            pos = 0;
